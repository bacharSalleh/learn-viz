...
`;class Rg extends D{constructor(){super(...arguments);P(this,"_hasAnimated",!1);P(this,"_cards",[]);P(this,"_deck",[]);P(this,"_current",0);P(this,"_flipped",!1);P(this,"_ratings",[]);P(this,"_done",!1);P(this,"_storageKey","lv-flashcard")}static get observedAttributes(){return["cards","shuffle","persist"]}connectedCallback(){super.connectedCallback(),this.adoptStyles(Og),this._build()}handleAttributeChange(){this.isConnected&&this._build()}animateIn(t){if(this._hasAnimated||(this._hasAnimated=!0,t))return;const n=this.root.querySelector(".fc-container");n&&(n.style.opacity="0",n.style.transition="opacity 0.4s ease-out",requestAnimationFrame(()=>{n.style.opacity="1"}))}_build(){if(this._cards=this.jsonAttr("cards",[]),this._ratings=[],this._done=!1,this._flipped=!1,this._current=0,!this._cards.length){this.render('<div class="fc-container"><p style="color:var(--lv-text-dim);text-align:center;">No cards provided.</p></div>');return}if(this._deck=this._cards.map((t,n)=>n),this.hasAttribute("shuffle"))for(let t=this._deck.length-1;t>0;t--){const n=Math.floor(Math.random()*(t+1));[this._deck[t],this._deck[n]]=[this._deck[n],this._deck[t]]}if(this.hasAttribute("persist"))try{const t=localStorage.getItem(this._storageKey);t&&(this._ratings=JSON.parse(t))}catch{}this._renderView(),this._attachKeyListener()}_attachKeyListener(){const t=n=>{if(!this.isConnected){document.removeEventListener("keydown",t);return}n.code==="Space"&&(n.preventDefault(),this._flip())};document.addEventListener("keydown",t)}_flip(){if(this._done)return;this._flipped=!this._flipped;const t=this.root.querySelector(".card");t&&t.classList.toggle("flipped",this._flipped)}_rate(t){const n=this._deck[this._current];if(this._ratings.push({index:n,rating:t}),this.dispatchEvent(new CustomEvent("lv-flashcard-rate",{detail:{index:n,rating:t,front:this._cards[n].front},bubbles:!0,composed:!0})),t==="hard"){const i=Math.min(this._current+4,this._deck.length);this._deck.splice(i,0,n)}if(this.hasAttribute("persist"))try{localStorage.setItem(this._storageKey,JSON.stringify(this._ratings))}catch{}this._current++,this._flipped=!1,this._current>=this._deck.length&&(this._done=!0),this._renderView()}_reset(){if(this._ratings=[],this.hasAttribute("persist"))try{localStorage.removeItem(this._storageKey)}catch{}this._build()}_esc(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}_renderView(){var a,o,c,l,d;if(this._done){const u=this._ratings.filter(g=>g.rating==="easy").length,h=this._ratings.filter(g=>g.rating==="ok").length,f=this._ratings.filter(g=>g.rating==="hard").length;this.render(`<div class="fc-container">
        <div class="done">
...
`,Er=["#3b82f6","#ef4444"];class Q0 extends D{constructor(){super(...arguments);P(this,"_hasAnimated",!1);P(this,"_nextClass",0)}static get observedAttributes(){return["data","model","resolution","interactive","c"]}connectedCallback(){super.connectedCallback(),this.adoptStyles(J0),this._build()}handleAttributeChange(){this.isConnected&&this._build()}animateIn(t){if(this._hasAnimated||(this._hasAnimated=!0,t))return;const n=this.root.querySelector("svg");n&&(n.style.opacity="0",n.style.transition="opacity 0.6s ease-out",requestAnimationFrame(()=>{n.style.opacity="1"}))}_build(){const t=this.jsonAttr("data",[{x:1,y:2,class:0},{x:2,y:3,class:0},{x:3,y:3,class:0},{x:1.5,y:1,class:0},{x:2.5,y:2,class:0},{x:5,y:5,class:1},{x:6,y:5,class:1},{x:5,y:6,class:1},{x:6,y:6,class:1},{x:5.5,y:4.5,class:1}]),n=this.getAttribute("model")||"linear",i=parseInt(this.getAttribute("resolution")||"50",10),s=this.hasAttribute("interactive"),a=parseFloat(this.getAttribute("c")||"1.0"),o=500,c=400,l={top:20,right:20,bottom:40,left:45},d=o-l.left-l.right,u=c-l.top-l.bottom;if(!t.length){this.render('<div class="db-container"></div>');return}const h=ne(t,L=>L.x),f=ne(t,L=>L.y),g=(h[1]-h[0])*.2||1,p=(f[1]-f[0])*.2||1,m=h[0]-g,b=h[1]+g,x=f[0]-p,v=f[1]+p,_=it().domain([m,b]).range([0,d]),y=it().domain([v,x]).range([0,u]),w=this._fitModel(t,n,a);let S="";const $=d/i,k=u/i;for(let L=0;L<i;L++)for(let C=0;C<i;C++){const z=m+(b-m)*(C+.5)/i,I=v-(v-x)*(L+.5)/i,F=w(z,I),q=Er[F>=.5?1:0];S+=`<rect x="${l.left+C*$}" y="${l.top+L*k}" width="${$+.5}" height="${k+.5}" fill="${q}" opacity="0.15"/>`}let A="";t.forEach(L=>{const C=l.left+_(L.x),z=l.top+y(L.y),I=Er[L.class];A+=`<circle class="point" cx="${C}" cy="${z}" r="5" fill="${I}"/>`});const T=_.ticks(6),M=y.ticks(6);let E="";if(T.forEach(L=>{const C=l.left+_(L);E+=`<g class="tick"><line x1="${C}" y1="${l.top+u}" x2="${C}" y2="${l.top+u+5}"/><text x="${C}" y="${l.top+u+18}" text-anchor="middle">${L}</text></g>`}),M.forEach(L=>{const C=l.top+y(L);E+=`<g class="tick"><line x1="${l.left-5}" y1="${C}" x2="${l.left}" y2="${C}"/><text x="${l.left-8}" y="${C+4}" text-anchor="end">${L}</text></g>`}),E+=`<line class="domain" x1="${l.left}" y1="${l.top}" x2="${l.left}" y2="${l.top+u}"/>`,E+=`<line class="domain" x1="${l.left}" y1="${l.top+u}" x2="${l.left+d}" y2="${l.top+u}"/>`,this.render(`<div class="db-container">
